...
const navSections = [
  {
    ...
    items: [
      ...
      { page: "pixel-stage" as Page, icon: "◈", label: "NEXUS", badge: "LIVE", badgeColor: "#00D4FF" },
  ...
  },
  {
    ...
    items: [
      ...
      { page: "workflows" as Page, icon: "◫", label: "Workflows", badge: "NEW", badgeColor: "#7B61FF" },
...
export default function Sidebar() {
  ...
  return (
    <motion.aside
      ...
      className="h-full glass-strong flex flex-col z-50 flex-shrink-0"
      style={{ width: 288 }}
    >
      {/* Logo */}
      <div className="px-6 py-5 border-b border-[#1A2744]/60">
        <div className="flex items-center gap-3">
          <motion.div
            ...
            animate={
              claudeConnected
                ? { filter: ["drop-shadow(0 0 8px rgba(0,212,255,0.4))", "drop-shadow(0 0 16px rgba(0,212,255,0.7))", "drop-shadow(0 0 8px rgba(0,212,255,0.4))"] }
          ...
          <div>
            <h1
              className={`font-semibold text-[18px] tracking-[6px] font-display ${claudeConnected ? "shimmer-text" : "gradient-text-full"}`}
            >
              ASTRAEO
            </h1>
        ...
        <div className="premium-card p-3.5 space-y-2.5">
          ...
          <div className="flex items-center justify-between text-xs">
            ...
            <motion.span
              ...
              className="text-[#00D4FF] font-mono font-semibold text-[11px]"
            >
...
function NavButton({ item, active, onClick }: { item: NavItem; active: boolean; onClick: () => void }) {
  return (
    <motion.button
      ...
      title={item.label}
      className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm relative group ${
        active
    ...
    >
      <span
        className={`text-lg w-5 text-center flex-shrink-0 transition-colors duration-150 ${
          active ? "text-[#00D4FF]" : "group-hover:text-[#E8ECF4]"
        }`}
      >
        {item.icon}
      </span>
